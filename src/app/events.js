...
document.getElementById("app").addEventListener("click",e=>{
  ...
  if(a==="begin"){const mode=startingMode(introDifficulty),openingCash=mode.cash;state.cash=openingCash;state.startingCash=openingCash;state.difficulty=mode.id;state.started=true;state.seen=["genesis"];log("Opening difficulty selected",`${mode.label} · ${fmtUsd(openingCash)}`,"finance");if(!state.startingGrant){state.points+=1;state.startingGrant=true;log("Genesis operator grant","+1 skill point")}state.speed=1;save();setTimer();render()}
  ...
  else if(a==="tab"){activeTab=v;render(false);window.scrollTo({top:0,behavior:"smooth"})}
  else if(a==="activate-hw")activateHardware(id);else if(a==="buy-hw")buyHardware(id,Number(v));else if(a==="buy-hw-btc")buyHardwareBtc(id,Number(v));else if(a==="sell-hw")sellHardware(id,Number(v));else if(a==="sell-hw-btc")sellHardwareBtc(id,Number(v));else if(a==="facility")upgradeFacility(id);else if(a==="region")moveRegion(id);else if(a==="buy-node")buyNode(Number(v));
