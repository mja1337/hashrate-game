...
function nodeHostPowered(){if(state.debt>0||state.policyLock)return false;if(state.node>=1)return true;const laptop=HARDWARE.find(x=>x.id==="laptop");return !!state.power&&!!laptop&&!hardwareOfflineReason(laptop)&&hardwarePoweredDownCount(laptop)<1}
