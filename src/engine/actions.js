...
function buyCooling(id){
  ...
  const trial=JSON.parse(JSON.stringify(state));trial.thermal.equipment[id]=(trial.thermal.equipment[id]||0)+1;if(!fleet(trial).within)return showToast("Electrical headroom required",`${item.name} adds ${item.watts.toLocaleString("en-US")} W of cooling demand. Pause miners or upgrade the facility first.`);
  state.cash-=item.cost;state.thermal.equipment[id]=(state.thermal.equipment[id]||0)+1;log(`Installed cooling: ${item.name}`,`-${fmtUsd(item.cost)} · +${fmtNum(item.coolingKw)} kW heat rejection`,"operations");showToast("Cooling installed",`${item.name} is now thermostatically controlled. The room will move toward its new target temperature over simulation time.`);save();renderMineContent();
}
