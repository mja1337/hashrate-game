...
function procurementTerms(h){const covid=state.time>=at("2020-03-12")&&state.time<at("2021-07-01"),early=h.era==="ASIC"&&state.time<at("2016-01-01"),hydro=h.era==="HYDRO ASIC";const days=(early?55:hydro?50:h.era==="ASIC"?28:h.era==="FPGA"?21:10)+(covid?42:0);return{days,risk:Math.min(.48,(early ? .24 : 0)+(hydro ? .12 : 0)+(covid ? .28 : 0)),label:covid?"COVID logistics risk":early?"Early ASIC batch risk":hydro?"Hydro freight risk":"Standard delivery"}}
...
function plannedFleetFits(id,qty){return plannedFleetProjection(id,qty).within}
...
function placeHardwareOrder(id,qty,btcCost=0){const h=HARDWARE.find(x=>x.id===id),terms=procurementTerms(h);if(!plannedFleetFits(id,qty))return showToast("Facility limit","Your installed fleet plus outstanding orders would exceed capacity.");state.procurementOrders.push({id,qty,due:state.time+terms.days*DAY,risk:terms.risk,slipped:false,label:terms.label});const paid=btcCost>0?fmtBtc(btcCost):fmtUsd(hardwareUnitCost(h)*qty);log(`Ordered ${qty} × ${h.name}`,`-${paid} · ${terms.days}-day lead time`,"fleet");showToast("Miner order placed",`${qty} × ${h.name}: ETA ${dateFmt(state.time+terms.days*DAY)} · ${Math.round(terms.risk*100)}% delivery-slippage risk.`);save();renderMineContent()}
function advanceProcurement(){state.procurementOrders=state.procurementOrders.filter(o=>{if(o.due>state.time)return true;const h=HARDWARE.find(x=>x.id===o.id);if(!h)return false;if(!o.slipped&&nextRand()<o.risk){const delay=14+Math.floor(nextRand()*35);o.due=state.time+delay*DAY;o.slipped=true;log(`${h.name} delivery slipped`,`${delay} additional days`);showToast("Delivery delayed",`${h.name} shipment slipped by ${delay} days (${o.label}).`);return true}state.inactiveHardware[o.id]=(state.inactiveHardware[o.id]||0)+Number(o.qty);log(`Miner delivery received`,`${o.qty} × ${h.name} awaiting activation`);showToast("Miners have arrived",`${o.qty} × ${h.name} is staged. Open Mine and activate the delivery before it can hash.`);renderFullQueued=true;return false})}
function activateHardware(id){const h=HARDWARE.find(x=>x.id===id),qty=Math.max(0,Math.floor(state.inactiveHardware?.[id]||0));if(!h||qty<1)return;const trial=JSON.parse(JSON.stringify(state));trial.hardware[id]=(trial.hardware[id]||0)+qty;if(!fleet(trial).within)return showToast("Activation blocked",`${qty} × ${h.name} no longer fits the active facility. Free capacity or upgrade the site.`);state.inactiveHardware[id]=0;state.hardware[id]=(state.hardware[id]||0)+qty;log(`Activated ${qty} × ${h.name}`,"Fleet deployment complete");showToast("Miners online",`${qty} × ${h.name} has been added to the active fleet.`);save();renderMineContent()}
...
function sellHardware(id,requested=1){
  const h=HARDWARE.find(x=>x.id===id),owned=state.hardware[id]||0;if(!h||owned<1||h.permanent)return;
  let qty=Math.max(1,Math.floor(Number(requested)||1));qty=Math.min(qty,owned);
  const value=resaleHardwareValue(h)*qty;state.hardware[id]-=qty;state.cash+=value;
...
function moveRegion(id){
  ...
  if(id==="sichuan"&&state.time>=at("2021-06-21"))return showToast("Region closed","Industrial Bitcoin mining is prohibited here after the 2021 crackdown.");
  ...
  state.cash-=cost;state.region=id;state.policyLock=null;state.power=state.debt<=0;log(`Fleet relocated to ${r.name}`,`-${fmtUsd(cost)}`);save();render();
}
...
function claimFaucet(){
  ...
  clearTimeout(faucetTimer);faucet=null;save();render();
}
...
function transactionPreview(button){
  ...
  if(action==="sell-hw"||action==="sell-hw-btc"){
    const h=HARDWARE.find(item=>item.id===id),owned=state.hardware[id]||0;if(!h||h.permanent||owned<1)return null;const qty=Math.min(Math.max(1,Math.floor(Number(button.dataset.value)||1)),owned),unit=resaleHardwareValue(h),value=unit*qty,payBtc=action==="sell-hw-btc",btc=payBtc?value/priceAt(state.time):0;
    return{...base,requested:qty,title:`Review miner sale · ${h.name}`,kicker:"Hardware sell · secondary-market quote",give:`${fmtCompactNumber(qty)} × ${h.name}`,giveSub:`${fmtHash(h.hash*qty)} physical hash leaves the fleet`,receive:payBtc?fmtBtc(btc):fmtUsd(value),receiveSub:payBtc?`${fmtUsd(value)} at ${fmtUsd(priceAt(state.time))}/BTC`:"Added to liquid fiat",reference:`${fmtUsd(unit)} resale per miner`,fees:"No modelled broker fee",after:payBtc?`${fmtCompactNumber(owned-qty)} miners · ${fmtBtc(state.wallets.hot+btc)} hot BTC`:`${fmtCompactNumber(owned-qty)} miners · ${fmtUsd(state.cash+value)} cash`,confirmLabel:"Confirm miner sale",confirmClass:"danger"}
  }
