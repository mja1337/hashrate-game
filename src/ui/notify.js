...
function feedbackLabel(kind){return({bad:"Action needed",warning:"Advance warning",blocked:"Cannot do that",milestone:"Milestone reached",success:"Completed",info:"Update"})[kind]||"Update"}
function toastMarkup(t){const linked=!!t.tab,kind=feedbackKind(t.title,t.kind),assertive=["bad","warning","blocked"].includes(kind);return `<div class="toast toast-${kind} ${linked?"toast-clickable":""}" role="${assertive?"alert":"status"}" aria-live="${assertive?"assertive":"polite"}" ${linked?`data-action="tab" data-value="${t.tab}"${t.anchor?` data-anchor="${t.anchor}"`:""}`:""}><div class="toast-copy"><small>${feedbackLabel(kind)}</small><b>${escapeHtml(t.title)}</b><span>${escapeHtml(t.message)}</span></div>${linked?`<i class="toast-go"><span>Open ${escapeHtml(t.tab)}</span> →</i>`:""}</div>`}
