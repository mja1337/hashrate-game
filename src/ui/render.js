...
function sandboxContinuationNote(){
  ...
  return `<section class="sandbox-decision"><h3>What continues after ${dateFmt(END)}</h3><p>The recorded historical feed ends here. Continuing keeps the same operation running for 100 more simulated years, driven by a deterministic model instead of recorded data.</p><dl><div><dt>Stops at the cutoff</dt><dd>No new historical chapters and no new hardware releases are invented after ${dateFmt(END)}. Nothing you read after this point is recorded history.</dd></div><div><dt>Continues as a model</dt><dd>Price, network hash rate, network difficulty, transaction activity, fees, chain size and block height continue through deterministic modelled projections.</dd></div><div><dt>Continues as protocol</dt><dd>Bitcoin's issuance schedule keeps running, so future halvings keep reducing mining income. The next is projected for ${dateFmt(next,true)}, taking the subsidy from ${fmtSubsidy(subsidyAt(END))} to ${fmtSubsidy(subsidyAt(next))} per block. Projected halving dates assume a constant ten-minute block interval, so treat the calendar as modelled and the direction as certain.</dd></div></dl></section>`;
}
...
function sectionPulse(){
  const fs=fleet(),monthly=monthlyCost(),offline=fs.offlineCount,runway=monthly.total?state.cash/monthly.total:Infinity,load=fs.cap?fs.kw/fs.cap*100:0,fleetCondition=fs.count?HARDWARE.reduce((sum,h)=>sum+(state.hardware[h.id]||0)*maintenanceCondition(h),0)/fs.count:100,learning=learningItem();
  const pages={
    ...
    energy:{purpose:"Control the operation's largest recurring cost.",situation:`The selected contract costs ${fmtUsd(powerRate(region()))}/kWh. At today's load, mining electricity costs about ${fmtUsd(dailyEnergyCostForWatts(fs.w*contractLoadFactor()))} per day.`,next:"Compare contract savings with shock risk, then confirm the monthly bill still fits your cash reserve.",why:"A powerful miner can still lose money when its electricity cost is greater than the BTC it is expected to earn.",metrics:[["All-in rate",`${fmtUsd(powerRate(region()))}/kWh`],["Energy / day",fmtUsd(dailyEnergyCostForWatts(fs.w*contractLoadFactor()))],["Contract",powerContract().name],["Bill so far",fmtUsd(state.bill)]]},
  ...
  const page=pages[activeTab];if(!page)return"";return `<section class="card section-pulse section-orientation" aria-labelledby="section-purpose-${activeTab}"><div class="orientation-intro"><div><div class="hero-kicker">What this page is for</div><h2 id="section-purpose-${activeTab}">${page.purpose}</h2></div><div class="orientation-guide"><div><span>Current situation</span><p>${page.situation}</p></div><div class="orientation-next"><span>Recommended next step</span><p>${page.next}</p></div><div><span>Why it matters</span><p>${page.why}</p></div></div></div><div class="metric-row">${page.metrics.map(([label,value])=>`<div class="metric"><div class="label">${label}</div><strong${label==="Simulation"?` data-live-simulation`:""}>${value}</strong></div>`).join("")}</div>${contextualHelp(activeTab)}</section>`;
}
...
function enhanceActiveTab(){
  ...
  const enhancer=TAB_ENHANCERS[activeTab];
...
const INTRO_SLIDES=[
  {kicker:"2009 to 2026 · recorded history",title:"Run one mining operation through the whole history of Bitcoin.",
    lead:"Every price, every difficulty retarget and every exchange collapse in this game is the real one, on the day it actually happened.",
    body:"You start in a spare room with one laptop and no idea what any of this will be worth. What happens to Bitcoin is already written. What happens to your operation is not.",
    facts:[["2009–2026","recorded history"],["1 laptop","where you start"],["17 years","to get through"]]},
  {kicker:"How operations die",title:"Mining earns BTC. The bills are due in cash.",
    lead:"Those are two different balances, and the gap between them is where most runs end.",
    body:"Electricity, rent and payroll arrive every month whether a block did or not. Sooner or later you will have to sell coins you would rather have kept, or find another way to pay. Miss the bill and the grid gives you until the next one; miss that too and the power goes off while the costs carry on.",
    facts:[["BTC","what you earn"],["Cash","what you owe"],["Monthly","when it falls due"]]},
  {kicker:"Before you start",title:"This is a simulation, and it takes its time.",
    lead:"A run covers seventeen years at whatever speed you choose, and you can pause at any point. There is no optimal build and no winning move.",
    body:"Most eras are unprofitable for a small operator — that is not a bug in your strategy, it is what the period was like, and staying solvent through it is the achievement. Every figure is labelled recorded, derived or modelled, and the Method tab carries the formula behind each one. When something goes against you the game explains why.",
    facts:[["Method","the full manual"],["Operator Score","how a run is judged"],["Pause anytime","the clock is yours"]]},
  {kicker:"Choose your run",title:"Pick a starting era and how much cash to begin with.",
    lead:"Campaign start decides the technology and the competition you face. Starting Liquidity is the cash available on day one.",
    body:"Earlier eras give you room to learn before the network gets serious. Later ones drop you in against industrial miners already running. More Starting Liquidity buys you time to make mistakes; it does not change historical prices, network data or how a run is scored.",
    facts:[["4 starts","Easy to Impossible"],["$1,500","default Starting Liquidity"],["$1,500,000","current maximum"]]}
];
...
function walletSetupModal(){
  ...
  if(w.step===0){
    return `<div class="modal-backdrop"><section class="modal" role="dialog" aria-modal="true" aria-labelledby="wallet-setup-title"><div class="modal-top"></div><div class="modal-body"><div class="modal-kicker">${dateFmt(state.time,true)} · one thing first</div><h2 id="wallet-setup-title">Your miner needs somewhere to send the rewards.</h2><p class="lead">Before the operation can run a single day it needs a wallet. A Bitcoin wallet manages private keys, and a private key is what proves you can spend the coins sent to it — whoever holds the key holds the BTC.</p><p>Wallets need unpredictable keys. This short ceremony uses dice to demonstrate that randomness: roll a handful yourself, then let the browser complete the 99-roll example. The result is educational only and cannot receive real bitcoin.</p><div class="modal-actions"><button class="action primary" data-action="wallet-setup-start">Create the example wallet</button><button class="action-link" data-action="wallet-setup-skip">Skip the demonstration</button></div></div></section></div>`;
  }
  if(w.step===1){
    const rolls=w.rolls,bits=(rolls.length*Math.log2(6)).toFixed(1),last=rolls[rolls.length-1],canFinish=rolls.length>=8;
    return `<div class="modal-backdrop"><section class="modal" role="dialog" aria-modal="true"><div class="modal-top"></div><div class="modal-body"><div class="modal-kicker">Creating an unpredictable key</div><h2>Add randomness with each roll.</h2><p class="lead">Each result adds entropy — randomness an attacker cannot predict. Real wallet software automates this securely; the dice make the idea visible.</p><div class="dice-roll-widget"><button class="die-face" data-action="dice-roll" aria-label="Roll the die">${last||"?"}</button><div class="dice-tally"><b>${rolls.length} / 99 rolls</b><span>${bits} / 256 bits of entropy</span></div></div><div class="modal-actions"><button class="action primary" data-action="dice-roll">Roll again</button>${canFinish?`<button class="action" data-action="dice-finish">Let the browser finish</button>`:""}</div></div></section></div>`;
  }
  const tier=WALLET_SOFTWARE[walletSoftwareTierAt(state.campaignStart)];
  return `<div class="modal-backdrop"><section class="modal" role="dialog" aria-modal="true"><div class="modal-top"></div><div class="modal-body"><div class="modal-kicker">Wallet ready · the clock starts next</div><h2>Your operation can now receive mining rewards.</h2><p class="lead">The rolls produced this illustrative private key:</p><div class="wallet-key-reveal">${w.keyHex}</div><p class="modal-note">Do not use this key for real bitcoin. A real wallet derives addresses and backups through additional cryptography that this demonstration does not implement.</p><p>Your ${dateFmt(state.campaignStart,true)} campaign uses <b>${tier.name}</b>: ${tier.desc}</p><p class="modal-note">Every page carries a panel saying what it is for and what to do next, and the Method tab explains where any number on screen comes from.</p><div class="modal-actions"><button class="action primary" data-action="wallet-setup-done">Open the operation</button></div></div></section></div>`;
}
...
function transactionImpact(transaction){if(transaction.action==="buy-btc")return transaction.id==="etf"?"Cash becomes price exposure, not withdrawable bitcoin.":"Cash becomes BTC held by the selected venue until you withdraw it.";if(transaction.action==="sell-btc")return"BTC leaves the selected position and spendable cash increases after the venue fee.";if(transaction.action==="buy-hw"||transaction.action==="buy-hw-btc")return"Payment leaves now. The miners reserve site capacity immediately but earn nothing until delivery and commissioning finish.";if(transaction.action==="sell-hw"||transaction.action==="sell-hw-btc")return"The retired miners leave the operation permanently and their resale value becomes the selected settlement asset.";if(transaction.action==="buy-strategy")return"Cash becomes a security position. It cannot pay a bill until sold and is not self-held BTC.";if(transaction.action==="sell-strategy")return"The security position falls and its modelled sale value becomes spendable cash.";if(transaction.action==="buy-backup-node")return`The deployment adds ${fmtUsd(BACKUP_NODE.monthly)} to recurring monthly costs after purchase.`;if(transaction.action==="buy-node")return"Cash buys verification infrastructure; it does not add mining hash rate or BTC.";return"The balances shown below become final when you confirm."}
function eventModal(){
  ...
  return `<div class="modal-backdrop"><section class="modal event-feedback-modal" role="dialog" aria-modal="true" aria-labelledby="event-title"><div class="modal-top"></div><div class="modal-body"><div class="modal-kicker">Historical chapter · ${dateFmt(eventTime)} · ${e.cat}</div><h2 id="event-title">${e.title}</h2><div class="event-reading"><section><span>What happened</span><p class="lead">${e.dek}</p></section><section><span>Why it mattered</span><p>${e.body}</p></section><section class="event-effect"><span>Effect on your operation</span><p>${eventGameplayEffect(e)}</p></section></div><div class="card-head event-market-head"><h3>What the market did next</h3><div class="meta">CONTEXT · NOT CAUSATION</div></div><div class="intro-grid"><div class="intro-fact"><b>${marketQuoted?fmtUsd(p0):"No USD market"}</b><span>BTC price on ${dateFmt(eventTime,true)}</span></div><div class="intro-fact"><b>${marketQuoted?fmtUsd(p1):"Not applicable"}</b><span>BTC price 30 days later</span></div><div class="intro-fact"><b style="color:${move===null?"var(--muted)":move>=0?"var(--green)":"var(--red)"}">${move===null?"—":`${move>=0?"+":""}${move.toFixed(1)}%`}</b><span>30-day market path</span></div></div><p class="modal-note">The price path is independent context. It does not claim this headline caused the move.</p><div class="modal-actions"><button class="action primary" data-action="close-event">Continue with this effect applied</button></div></div></section></div>`
}
/* A LOSS IS NOT A NOTIFICATION.

   This modal exists because a toast was the wrong size for the message. It reports four things
   in a fixed order — what left, where it left from, why it was possible, and what prevents it
   next time — and the fourth is the one worth reading. The player cannot undo the loss; they
...
function settlementModal(){
  ...
  return `<div class="modal-backdrop"><section class="modal" role="dialog" aria-modal="true" aria-labelledby="settlement-title"><div class="modal-top"></div><div class="modal-body"><div class="modal-kicker">Monthly settlement · time paused</div><h2 id="settlement-title">The operation is ${fmtUsd(short)} short.</h2><p class="lead">Choose how the business meets its ${fmtUsd(p.due)} power, rent and finance bill. Ignoring it is no longer a winning strategy.</p><div class="intro-grid"><div class="intro-fact"><b>${fmtUsd(state.cash)}</b><span>liquid cash</span></div><div class="intro-fact"><b>${state.time>=MARKET?fmtBtc(btcNeeded):"No market"}</b><span>self-held BTC needed</span></div><div class="intro-fact"><b>${fmtUsd(saleable)}</b><span>estimated miner resale</span></div><div class="intro-fact"><b>${strike} / 3</b><span>next receivership strike</span></div></div><div class="venue-grid"><article class="venue"><div class="risk medium">TREASURY RESCUE</div><h3>Sell on an exchange</h3><p>Deposit self-held BTC onto a venue and sell it there at that exchange's own price and fee. The bill clears the moment cash covers it.</p><button class="action small primary" data-action="settle-btc" ${state.time<MARKET||controlled()<=1e-12?"disabled":""}>Go to the exchange</button></article><article class="venue"><div class="risk medium">FLEET RESCUE</div><h3>Liquidate inefficient miners</h3><p>Sell the least hash-efficient equipment first. If it is not enough, choose again.</p><button class="action small" data-action="settle-liquidate" ${saleable<=0?"disabled":""}>Raise up to ${fmtUsd(saleable)}</button></article><article class="venue"><div class="risk high">15% ORIGINATION PENALTY</div><h3>Emergency bridge finance</h3><p>Receive the missing cash now; the larger principal joins project debt.</p><button class="action small" data-action="settle-bridge" ${state.time<MARKET?"disabled":""}>Borrow ${fmtUsd(short*1.15)}</button></article><article class="venue"><div class="risk medium">GRACE UNTIL THE NEXT BILL</div><h3>Miss this month's bill</h3><p>Immediate effect: ${fmtUsd(short)} goes into arrears and the site keeps running. Consequence: if the arrears are still owed at the next bill date, power and internet are cut until they are paid, and the month is recorded as a rescue.</p><button class="action small" data-action="settle-defer" ${state.debt>0?"disabled":""} title="${state.debt>0?"You are already carrying arrears; the grid is cut at the next bill date":""}">Carry ${fmtUsd(short)} into arrears</button></article><article class="venue"><div class="risk high">FAIL FORWARD · STRIKE ${strike}</div><h3>Enter receivership</h3><p>Clear the bill, but lose ${Math.round(Math.min(.25,.1+(strike-1)*.05)*100)}% of self-held BTC and 25% of installed miners.</p><button class="action small danger" data-action="settle-receivership">${strike>=3?"End scored campaign":"Restructure and continue"}</button></article></div><p class="modal-note" style="margin-top:12px">A clean monthly settlement earns solvency points. Emergency BTC sales, liquidation and bridge finance keep the run alive but do not count as a clean solvent month.</p></div></section></div>`;
}
const alpha26SettlementModal=settlementModal;
settlementModal=function(){
  const short=Math.max(0,state.pendingSettlement.due-state.cash),plan=fleetLiquidationPlan(short),markup=alpha26SettlementModal(),haircut=Math.round(Math.min(.25,.1+state.operator.restructures*.05)*100);
  return markup.replace("The operation is "+fmtUsd(short)+" short.",`${fmtUsd(short)} is needed before the operation can continue.`).replace("Choose how the business meets its "+fmtUsd(state.pendingSettlement.due)+" power, rent and finance bill. Ignoring it is no longer a winning strategy.",`The ${fmtUsd(state.pendingSettlement.due)} operating bill is due now and only ${fmtUsd(state.cash)} is available in cash. Time remains paused until you choose. Missing the bill is a route, not a failure state — it costs you the grid if it runs past the next bill date.`).replace('<div class="venue-grid">','<div class="venue-grid settlement-choice-grid">').replace("Sell the least hash-efficient equipment first. If it is not enough, choose again.",plan.qty?`Immediate effect: sell ${liquidationPlanLabel(plan)} for ${fmtUsd(plan.total)}.${plan.covered?" The bill will clear.":` ${fmtUsd(plan.remaining)} will still be due.`}`:"Immediate effect: no non-permanent mining hardware remains to sell.").replace("Deposit self-held BTC onto a venue and sell it there at that exchange's own price and fee. The bill clears the moment cash covers it.","Immediate effect: move BTC to an exchange and sell enough to raise cash. Consequence: the month is recorded as a rescue rather than a clean settlement.").replace("Receive the missing cash now; the larger principal joins project debt.",`Immediate effect: receive ${fmtUsd(short)} cash. Consequence: ${fmtUsd(short*1.15)} is added to project debt, including the 15% penalty.`).replace(/Clear the bill, but lose \d+% of self-held BTC and 25% of installed miners\./,`Immediate effect: lose ${haircut}% of self-held BTC and 25% of installed miners. Consequence: strike ${state.operator.restructures+1} of 3; the third ends the scored campaign.`).replace(/Raise up to [^<]+/,plan.qty?`Sell ${fmtCompactNumber(plan.qty)} miner${plan.qty===1?"":"s"} · raise ${fmtUsd(plan.total)}`:"No saleable miners");
};
function endModal(){
