...
function sidebar(){
  ...
  return `<aside class="sidebar"><div class="story-head"><h2>The Bitcoin story</h2><span class="story-count">${past.length} / ${EVENTS.length} chapters</span></div><article class="story-feature"><div class="story-date">${dateFmt(at(feature.date))} · ${feature.cat}</div><h3>${feature.title}</h3><div class="dek">${feature.dek}</div><p>${feature.body}</p>${feature.url?`<div class="story-source"><a href="${feature.url}" target="_blank" rel="noopener">${feature.id==="genesis"?"Read the Bitcoin whitepaper":`Source: ${escapeHtml(feature.src||"")}`}</a></div>`:""}</article><div class="story-list">${past.slice(1,11).map(e=>`<article class="story-item" data-action="story" data-id="${e.id}" tabindex="0"><span class="cat">${e.cat}</span><time>${dateFmt(at(e.date))}</time><h4>${e.title}</h4></article>`).join("")}</div></aside>`
}
