...
const INTRO_SLIDES=[
  {kicker:"Chapter one · the spare room",title:"One laptop. An uncertain future.",
    lead:"What happens to Bitcoin is already written. What happens to your operation is not.",
    body:"Start in 2009 with a laptop and $1,500. Run the software, watch the work accumulate, and keep the electricity paid. Your first goal: make it through the first operating month.",
...
function introModal(){
  ...
  return `<div class="modal-backdrop"><section class="modal" role="dialog" aria-modal="true" aria-labelledby="intro-title"><div class="modal-top"></div><div class="modal-body"><div class="modal-kicker">${s.kicker}</div><h2 id="intro-title">${s.title}</h2><p class="lead">${s.lead}</p><p>${selectedMode.id==="easy"?`Start in 2009 with your own laptop. Run the software, watch the work accumulate, and keep the electricity paid. Your first goal: make it through the first operating month.`:`${selectedMode.desc} ${selectedMode.id==="medium"?"Keep your laptop running and cover the first operating bill.":"Start paused and plan your first upgrade."}`}</p><div class="intro-grid">${s.facts.map(([b,sp])=>`<div class="intro-fact"><b>${b}</b><span>${sp}</span></div>`).join("")}</div><details class="intro-customise" ${introStep?"open":""}><summary>Customise your run</summary>${capital}</details><p class="modal-note">Recorded Bitcoin history, with derived and modelled operations. Pause anytime; the Method tab explains the numbers.</p><div class="modal-actions">${i>0?`<button class="action" data-action="intro-back">Back</button>`:""}${last?`<button class="action primary" data-action="begin">${selectedMode.id==="easy"?"Start your laptop":"Open your operation"}</button>`:`<button class="action primary" data-action="intro-next">Next</button>`}<span class="modal-note">${selectedMode.label} · ${dateFmt(selectedMode.start)}</span></div></div></section></div>`
}
