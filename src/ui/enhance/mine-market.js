...
function fleetServicingVisual(){
  ...
  const hireNudge=!needsHelp||technicians>0?"":staffHiringAvailable()?`<div class="maintenance-nudge"><span>No field technicians on payroll — you service the fleet yourself. Labour is free, but only one job at a time and a fumbled repair can damage the machine.</span><button class="action small primary" data-action="hire-staff" data-id="fieldtech">Hire field technician · ${fmtUsd(1200)}/month</button></div>`:`<div class="maintenance-nudge"><span>You are maintaining the fleet yourself: free labour, one job at a time, and a real chance of damaging a machine. Hire field technicians once you reach a Light industrial unit (tier 3).</span></div>`;
  const rows=owned.map(h=>{
...
function floorViewToggle(){
  const on=state.floorView==="3d",blocked=typeof floor3dUnavailableReason==="function"?floor3dUnavailableReason():"";
  return `<div class="floor-view-switch"><span>Floor view</span>
    <button class="action small ${on?"":"primary"}" data-action="floor-view" data-value="2d" aria-pressed="${!on}">Flat</button>
    <button class="action small ${on?"primary":""}" data-action="floor-view" data-value="3d" aria-pressed="${on}"${blocked?` disabled title="${blocked}"`:""}>3D</button>
    ${blocked?`<small class="modal-note">${blocked}</small>`:""}</div>${on?`<div class="floor-3d-mount" role="img" aria-label="Three-dimensional view of the mining floor"></div>`:""}`;
}
...
function incomingFleetVisual(){
  ...
  if(!transit.length&&!delivered.length&&!commissioning.length)return"";
...
function enhanceMine(){
  ...
  const hero=tabCommandVisual("mine"),floorAndService=miningFloorVisual()+fleetServicingVisual();
  const incomingFleet=incomingFleetVisual();
  grid.insertAdjacentHTML("afterbegin",banner+metrics+incomingFleet+hero+floorAndService);
  ...
  if(state.floorView==="3d"&&typeof mountFloor3d==="function")mountFloor3d();
}
