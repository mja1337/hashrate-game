...
The silent conversion is now announced. When settlement sells BTC because cash fell short, it says what it sold, what that raised, at what price, that the standing instruction is why, and what is left. It escalates when the sale takes half or more of the remaining treasury: **Treasury nearly emptied to pay the bill** rather than **Bill covered by selling BTC**. Settlement notices also respect silent ticks now, so returning to a hidden tab no longer fires a month of toasts at once.

Fixing the grace month exposed a second problem in the forecast: it assumed miners were off the instant arrears existed, understating the energy bill for exactly the month the player most needs the number to be right. It now asks whether the grid is actually cut.
